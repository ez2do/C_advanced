...
void addEdgeOneWay(Graph g, int v1, int v2, int distance){
	JRB node = jrb_find_int(g.edges, v1);

	if(node != NULL){
		JRB tree = (JRB)jval_v(node->val);
		if(!jrb_find_int(tree, v2))
			jrb_insert_int(tree, v2, new_jval_i(distance));
	}
	else{
		JRB tree = make_jrb();
...
void addEdge(Graph g, int v1, int v2, int distance){
	addEdgeOneWay(g, v1, v2, distance);
	addEdgeOneWay(g, v2, v1, distance);
}
