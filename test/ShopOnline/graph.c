...
void addVertex(Graph g, int id, int weight){
	...
	if(node != NULL)
		...
		jrb_insert_int(g.vertices, id, new_jval_i(weight));
}
...
void addEdgeOneWay(Graph g, int v1, int v2, int length){
	...
	if(node != NULL){
		...
		if(!ptr)
			jrb_insert_int(tree, v2, new_jval_i(length));
		else{
			ptr->val = new_jval_i(length);
		}
	}
	else{
		JRB tree = make_jrb();
...
JRB dijkstra(Graph g, int start, int stop){
	...
	if(startNode == NULL)
		...
		jrb_traverse(ptr, g.vertices){
			ptr->val = new_jval_i(INFINITY);
			jrb_insert_int(parentList, jval_i(ptr->key), new_jval_i(-1));
		}
		startNode->val = new_jval_i(0);
